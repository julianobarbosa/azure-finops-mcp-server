...
def get_unattached_disks(
    ...
    unattached_disks = []
    disk_categories = {"orphaned": [], "pvc": [], "aks_managed": []}
    ...
    return {
        ...
        "statistics": {
            "total_unattached": len(disk_categories["orphaned"])
            + len(disk_categories["pvc"])
            + len(disk_categories["aks_managed"]),
...
def fetch_unattached_disks(compute_client: Any, regions: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    """
...
def categorize_disks(disks: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    """
    categories = {"orphaned": [], "pvc": [], "aks_managed": []}
    ...
    for disk in disks:
        ...
        is_aks = any(resource_group.startswith(pattern) for pattern in config.managed_resource_group_patterns)
...
def get_detailed_disk_audit(
    credential, subscription_id: str, regions: Optional[List[str]] = None
) -> Tuple[Dict[str, Any], ApiErrors]:
    """
    ...
    """
    api_errors: ApiErrors = {}
    audit_results = {"summary": {}, "orphaned_disks": [], "pvc_disks": [], "aks_managed_disks": [], "cost_analysis": {}}
...
def generate_disk_recommendations(audit_results: Dict[str, Any]) -> List[str]:
    ...
    if orphaned_count > 0:
        recommendations.append(f"Delete {orphaned_count} orphaned disks to save {format_cost(orphaned_cost)}/month")

    pvc_count = summary.get("pvc_count", 0)
    if pvc_count > 10:
        recommendations.append(f"Review {pvc_count} PVC disks - consider cleaning up unused Kubernetes volumes")

    aks_count = summary.get("aks_managed_count", 0)
    if aks_count > 0:
        recommendations.append(f"Found {aks_count} AKS-managed disks - verify AKS cluster health")

    total_cost = summary.get("total_monthly_cost", 0)
    if total_cost > 100:
        annual_savings = calculate_yearly_cost(total_cost)
        recommendations.append(f"Total potential savings: {format_cost(annual_savings)}/year")
