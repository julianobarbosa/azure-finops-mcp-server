    ...
    try:
        ...
        for public_ip in network_client.public_ip_addresses.list_all():
            ...
            if public_ip.ip_configuration is None:
                ...
                # Add cost estimate
                ip_info["monthly_cost"] = estimate_public_ip_cost(ip_info["sku"], ip_info["allocation_method"])
...
def analyze_network_usage(
    ...
    """
    api_errors: ApiErrors = {}
    analysis = {"public_ips": {}, "network_security_groups": {}, "virtual_networks": {}, "recommendations": []}
    ...
    if not nsg_errors:
        ...
        unused_nsgs = [nsg for nsg in nsg_result.get("network_security_groups", []) if nsg.get("status") == "Unused"]
        if unused_nsgs:
            analysis["recommendations"].append(f"Review {len(unused_nsgs)} unused Network Security Groups for deletion")
    else:
