...
def get_budget_data(credential, subscription_id: str) -> Tuple[Dict[str, Any], ApiErrors]:
    ...
    budget_info = {"budgets": [], "summary": {}, "alerts": []}
    ...
    try:
        ...
        for budget in budgets:
            budget_detail = {
                ...
                "time_period": {
                    "start_date": (
                        budget.time_period.start_date.isoformat()
                        if budget.time_period and budget.time_period.start_date
                        else None
                    ),
                    "end_date": (
                        budget.time_period.end_date.isoformat()
                        if budget.time_period and budget.time_period.end_date
                        else None
                    ),
                },
            }
            ...
            if hasattr(budget, "current_spend") and budget.current_spend:
                ...
                budget_detail["percentage_used"] = (
                    round((budget_detail["current_spend"]["amount"] / budget_detail["amount"]) * 100, 2)
                    if budget_detail["amount"] > 0
                    else 0
                )
            ...
            if hasattr(budget, "forecast_spend") and budget.forecast_spend:
                ...
                budget_detail["forecast_percentage"] = (
                    round((budget_detail["forecast_spend"]["amount"] / budget_detail["amount"]) * 100, 2)
                    if budget_detail["amount"] > 0
                    else 0
                )
...
def analyze_spending_trends(credential, subscription_id: str, months: int = 6) -> Tuple[Dict[str, Any], ApiErrors]:
    """
    ...
    """
    api_errors: ApiErrors = {}
    trends = {"monthly_spend": [], "average_monthly": 0, "trend": "stable", "projection": {}}
