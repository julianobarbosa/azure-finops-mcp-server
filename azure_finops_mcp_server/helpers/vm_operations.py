...
def get_vm_instance_view_batch(compute_client: ComputeManagementClient, vm_list: List[Any]) -> Dict[str, Any]:
    """
...
def get_stopped_vms(
    ...
    try:
        ...
        result = {"stopped_vms": stopped_vms, "statistics": statistics}
...
def generate_vm_recommendations(stopped_vms: List[Dict[str, Any]], total_waste: float) -> List[str]:
    ...
    if vm_count > 0:
        recommendations.append(f"Consider deleting {vm_count} stopped VMs to save {format_cost(total_waste)}/month")

    if vm_count > 10:
        recommendations.append("High number of stopped VMs detected - consider implementing auto-shutdown policies")

    # Check for expensive stopped VMs
    expensive_vms = [vm for vm in stopped_vms if vm.get("estimated_monthly_cost", 0) > 500]
    if expensive_vms:
        recommendations.append(f"Found {len(expensive_vms)} high-cost stopped VMs (>{format_cost(500)}/month each)")

    if total_waste > 1000:
        annual_savings = calculate_yearly_cost(total_waste)
        recommendations.append(f"Significant savings opportunity: {format_cost(annual_savings)}/year")
