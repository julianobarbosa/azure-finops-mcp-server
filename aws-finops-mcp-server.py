...
mcp = FastMCP("AWSFinOpsMCPServer_SinglePeriod")
        ...
        if start_date_iso and end_date_iso:
            try:
                ...
                if period_start_date > period_display_end_date:
                    return {"profile_name": profile_name, "status": "error", "message": "Start date cannot be after end date."}
            except ValueError:
                return {"profile_name": profile_name, "status": "error", "message": "Invalid date format. Use YYYY-MM-DD."}
        elif time_range_days is not None:
            if time_range_days <= 0:
                return {"profile_name": profile_name, "status": "error", "message": "time_range_days must be positive."}
