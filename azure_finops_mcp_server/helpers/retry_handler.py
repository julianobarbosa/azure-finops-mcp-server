...
class RetryConfig:
    ...
    def from_config(cls) -> "RetryConfig":
        ...
        config = get_config()
        return cls(max_retries=config.max_retries, backoff_factor=config.retry_backoff_factor)
...
class RetryHandler:
    ...
    def __init__(self, config: Optional[RetryConfig] = None):
        ...
        self.stats = {"total_retries": 0, "successful_retries": 0, "failed_operations": 0}
    ...
    def calculate_backoff(self, attempt: int) -> float:
        ...
        """
        backoff = min(self.config.initial_backoff * (self.config.backoff_factor**attempt), self.config.max_backoff)
